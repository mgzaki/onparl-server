...
resource "aws_s3_bucket" "web_client" {
  bucket = "onparl-web-client-${random_id.bucket_suffix.hex}"
}
...
resource "aws_cloudfront_origin_access_identity" "web_client" {
  comment = "OAI for Signal Web Client"
}
    ...
    Statement = [
      {
        Effect = "Allow"
        Principal = {
          AWS = "arn:aws:iam::cloudfront:user/CloudFront Origin Access Identity ${aws_cloudfront_origin_access_identity.web_client.id}"
        }
        Action   = "s3:GetObject"
...
resource "aws_cloudfront_distribution" "web_client" {
  ...
  # Forward WebSocket connections to ALB
  ordered_cache_behavior {
    path_pattern     = "/ws-signal*"
    allowed_methods  = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "ALB-${var.alb_dns_name}"
